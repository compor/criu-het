...
class Converter:
    ...
    def get_symbol_addr(self, binary, symbol):
        if len(binary_symbols) == 0:
            ...
            _stdout = subprocess.check_output(
                ["nm", binary], stderr=PIPE, encoding="utf-8"
            )
    ...
    def read_regs_from_memory(
        self, binary, architecture, pagemap_file, pages_file, struct_def
    ):
        rrfm_time = time.time()
        ...
        regs = self.read_struct_from_pages(
            pages_file, region_offset, struct_def
        )
        rrfm_time3 = time.time()

        het_log(
            "rrfm",
            rrfm_time1 - rrfm_time,
            rrfm_time2 - rrfm_time1,
            rrfm_time3 - rrfm_time2,
        )
        return regs

    def read_tls_from_memory(
        self, binary, architecture, pagemap_file, pages_file
    ):
        rrfm_time = time.time()
    ...
    def __add_target_region(
        ...
        assert page_nbr != -1
        pagemap_img["entries"] = (
            pages_list[:idx] + [pgmap_tmpl] + pages_list[idx:]
        )
    ...
    @abstractmethod
    def get_target_core(
        self, arch, binary, pages_file, pagemap_file, core_file
    ):
        pass
    ...
    def __recode_pid(
        ...
        dest_core = self.get_target_core(
            arch, binary, pages_file, pagemap_file, core_file
        )
        time_core = time.time()
        dest_files = self.get_target_files(
            files_file, mm_file, path_append, root_dir
        )  # must be after get_target_core
        time_files = time.time()

        bname = os.path.basename(pages_file)
        dst_file = os.path.join(outdir, bname)
        dest_mm, dest_pagemap, dest_pages_path = self.get_target_mem(
            mm_file, pagemap_file, pages_file, dst_file
        )
        time_mem = time.time()
        ...
        for fl in onlyfiles:
            ...
            if "pages" in fl:  # just copy to target file
                het_log(
                    "copy of pages file (mem) already done above... we modified the final copy directly"
                )
            else:
        ...
        print(
            pid,
            (time_path - time_start),
            (time_core - time_path),
            (time_files - time_core),
            (time_mem - time_files),
            (time_copy - time_mem),
        )
    ...
    def recode(self, arch, directory, outdir, path_append, root_dir):
        ...
        onlyfiles = [
            ...
            if (isfile(join(directory, f)) and "img" in f)
        ]
        pstree_file = None
        files_file = None
        handled_files = []
        ...
        rec_t3 = time.time()
        het_log(
            "recode", (rec_t1 - rec_t0), (rec_t2 - rec_t1), (rec_t3 - rec_t2)
        )
...
class X8664Converter(Converter):
    ...
    def convert_to_dest_core(
        self, pgm_img, dest_regs, dest_tls
    ):  # , old_stack_tmpl, new_stack_tmpl):
        ...
        reg_dict["fs_base"] = hex(
            int(src_info["clear_tid_addr"], 16) - 56
        )  # "0x821460" #FIXME!!!
    ...
    def get_target_core(
        self, architecture, binary, pages_file, pagemap_file, core_file
    ):
        # old_stack_tmpl, new_stack_tmpl = self.__move_stack(pages_file, pagemap_file, core_file, mm_file)
        target_start = time.time()
        dest_regs = self.read_regs_from_memory(
            binary, architecture, pagemap_file, pages_file, X86Struct
        )
        target_regs = time.time()
        dest_tls = self.read_tls_from_memory(
            binary, architecture, pagemap_file, pages_file
        )
        ...
        dst_core = self.convert_to_dest_core(
            src_core, dest_regs, dest_tls
        )  # , old_stack_tmpl, new_stack_tmpl)
        target_dst = time.time()
        het_log(
            "get_target_core x86_64",
            (target_regs - target_start),
            (target_tls - target_regs),
            (target_src - target_tls),
    ...
    def get_target_files(self, files_path, mm_file, path_append, root_dir):
        ...
        fid, idx, bin_path = self.get_binary_info(
            files_path, mm_file, path_append
        )
        tmp_root_dir = root_dir
    ...
    def get_vdso_template(self):
        ...
        vdso_path = os.path.join(
            dir_path, "templates/", "x86_64_vdso.img.tmpl"
        )
    ...
    def get_target_mem(self, mm_file, pagemap_file, pages_file, dest_path):
        ...
        ret_size = self.remove_region_type(
            mm_img, pagemap_img, page_tmp, original_size, "VDSO"
        )
        if ret_size > 0:
            ret_size = self.add_target_region(
                mm_img, pagemap_img, page_tmp, ret_size, "VDSO"
            )
            if ret_size > 0:
        ...
        gtm_t4 = time.time()
        ret_size = self.remove_region_type(
            mm_img, pagemap_img, page_tmp, original_size, "VVAR"
        )
        if ret_size > 0:
            ret_size = self.add_target_region(
                mm_img, pagemap_img, page_tmp, ret_size, "VVAR"
            )
            if ret_size > 0:
                original_size = ret_size

        ret_size = self.add_target_region(
            mm_img, pagemap_img, page_tmp, original_size, "VSYSCALL"
        )
...
class Aarch64Converter(Converter):
    ...
    def __get_rlimits(self):
        return [
            ...
            {"cur": 515133, "max": 515133},
            {
                "cur": 1024,
                "max": 100000,
            },  # x86_64 is {"cur": 8192, "max": 100000},
            {
                "cur": 18446744073709551615,
                "max": 18446744073709551615,
            },  # x86_64 is {"cur": 65536, "max": 65536},
            {"cur": 18446744073709551615, "max": 18446744073709551615},
            {"cur": 18446744073709551615, "max": 18446744073709551615},
            {"cur": 515133, "max": 515133},
            {"cur": 819200, "max": 819200},
    ...
    def get_target_core(
        ...
        target_start = time.time()
        dest_regs = self.read_regs_from_memory(
            binary, architecture, pagemap_file, pages_file, Aarch64Struct
        )
        target_regs = time.time()
        dest_tls = self.read_tls_from_memory(
            binary, architecture, pagemap_file, pages_file
...
def test_convert_core():
    ...
    pagemap_file = (
        "/share/karaoui/criu-project/dumps/hello/new-dump/pagemap-"
        + str(pid)
        + ".img"
    )
    core_file = (
        "/share/karaoui/criu-project/dumps/hello/new-dump/core-"
        + str(pid)
        + ".img"
    )
    architecture = 1
